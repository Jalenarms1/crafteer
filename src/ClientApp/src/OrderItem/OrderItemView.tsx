...
const OrderItemView = ({OnSave, ExistingItem, isShowingBackButton = true}: OrderItemViewParams) => {
    const navigate = useNavigate()
    const {orderItem, setOrderItem, isShowingBackImage, setIsShowingBackImage, selectedDesignItem, setSelectedDesignItem} = useOrderItem(ExistingItem)
  ...
  return (
    <div className='bg-zinc-100 w-full h-screen flex overflow-y-hidden'>
        <div className='w-3/4 h-screen shadow-zinc-700 flex flex-col items-center '>
            {isShowingBackButton && <div className='flex items-center bg-zinc-100 border-b border-zinc-200 w-full p-2 gap-2'>
                <IoArrowBack className='text-black'/>
                <p onClick={() => navigate(-1)} className="text-black">Back</p>
            </div>}
            <div className="w-full flex justify-between ">
            <div className="bg-white w-3/5 flex items-center justify-center overflow-hidden h-[600px] flex-1 flex-col relative mx-auto">
                

                {/* <img src={whiteTeeImg} className=''/> */}
                <img src={isShowingBackImage ? selectedAsset.backImage : selectedAsset.image} className='min-w-[800px] max-w-[800px] h-[800px]'/>
                ...
                return (<div onClick={() => selectedDesignItem?.id != i.id ? setSelectedDesignItem(i) : setSelectedDesignItem(null)} className={` z-[99] absolute   flex w-[225px] `} style={{top: imagePositionDisplayList.find(f => f.position == i.designPosition)?.top, right: imagePositionDisplayList.find(f => f.position == i.designPosition)?.right}}>
                    <div className={` w-full flex ${i?.designPosition == "Top-Right" ? "justify-end" : i?.designPosition == "Fill" ? "justify-center" : "justify-start"} `}>
                        <div className='relative w-fit pl-3'>
        ...
        <div className='w-1/4 h-screen bg-orange-50 p-5 flex flex-col gap-10'>
            <div className='flex flex-col gap-5'>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <div className="flex items-center gap-2">
                        {selectedAsset.sizeGroups.map(s => (
                            <p onClick={() => {
                                if(!orderItem) return
                                setOrderItem(p => {
                                    return {
                            ...
                            }} className={` ${orderItem?.sizeGroup == s ? "bg-orange-950 text-white" : "border border-solid border-zinc-500"} rounded-md px-4  cursor-pointer hover:shadow-sm hover:shadow-zinc-700`}>{s}</p>
                        ))}
            ...
            <button onClick={() => {
                ...
                let errList: {field: string, err: string}[] = []
                if(!parseInt(orderItem?.quantity?.toString() ?? "")) {
                    errList = [...errors.filter(f => f.field != "quantity"), {field: "quantity", err: "Enter a valid number"}]
                    setErrors(e => {
                        return errList
                    })
                }
